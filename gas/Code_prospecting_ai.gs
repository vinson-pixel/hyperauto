...
function generateTalkScript(rowIndex) {
  ...
  var sys = '電話営業のプロです。マルケン電工（愛知県の電気工事会社）の営業担当が使うトークスクリプトを作成します。JSONのみ出力。';
  var user =
    '以下の会社への電話営業トークスクリプトをJSONで作成してください。\n' +
    '会社名: ' + company + '\n業種: ' + industry + '\nURL: ' + url + '\nメモ: ' + memo + '\n\n' +
    '【重要ルール】\n' +
    '・冒頭は必ず「お世話になっております。株式会社マルケン電工の〇〇です。」から始める\n' +
    '・受付突破: 担当者名が分かれば名指し、不明なら部署推測（「設備管理のご担当の方」等）\n' +
    '・「ご担当者をお願いします」は絶対NG\n' +
    '・折り返し依頼 → 担当者名を聞く → いる時間帯を聞く → 「では〇時ごろ改めます」\n' +
    '・「資料送って」→ 強気で「直接5分だけお話しさせてください」とアポに持ち込む\n' +
    '・新人でも実践できる具体的なセリフ\n\n' +
    'JSON形式:\n' +
    '{"opening":"受付突破トーク","pitch":"担当者への提案トーク30秒","objections":[{"q":"断り文句","a":"切り返し"},{"q":"断り文句","a":"切り返し"},{"q":"断り文句","a":"切り返し"}],"close":"クロージング・日程提案"}';

  // Grok → Claude の順で試みる（Grokを優先：月次コスト節約）
  var result = null;
  if (getProp('XAI_API_KEY')) {
    try { result = callGrokJSON(sys, user, 'grok-3-mini-fast-beta'); } catch(e) { Logger.log('Grok失敗: ' + e); }
  }
  if (!result && getProp('CLAUDE_API_KEY')) {
    try { result = callClaudeJSON(sys, user, 'claude-haiku-4-5-20251001'); } catch(e) { Logger.log('Claude失敗: ' + e); }
  }

  if (!result) return { error: 'スクリプト生成失敗（XAI_API_KEY / CLAUDE_API_KEY を確認してください）' };
